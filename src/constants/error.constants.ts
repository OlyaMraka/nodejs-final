export const MiddlewareConstants = {
    ACCESS_DENIED: "Access denied",
    INVALID_ID: "Invalid ID",
    NO_ACCESS_TOKEN: "No access token provided",
    NO_ACCESS_TOKEN_FOUND: "No access token found",
    NO_REFRESH_TOKEN: "No refresh token provided",
    NO_REFRESH_TOKEN_FOUND: "No refresh token found"
}

export const ServiceConstants = {
    INVALID_EMAIL_OR_PASSWORD: "Invalid email or password!",
    CAR_ADS_NOT_FOUND: "Car ads not found!",
    CAR_AD_NOT_FOUND: "Car Ad not found!",
    INVALID_AD_DESCRIPTION: "Invalid ad description!",
    INVALID_CAR_BRAND: "Invalid car brand!",
    INVALID_CAR_MODEL: "Invalid car model!",
    USER_NOT_FOUND: "User not found!",
    BASIC_ACCOUNT_ALLOWS_ONE_AD: "A basic account allows you to publish one ad!",
    STATISTICS_AVAILABLE_FOR_PREMIUM: "Statistics are available only for premium accounts!",
    USER_ALREADY_EXISTS: "User already exists!",
}

export const EmailTopicsConstants = {
    NEW_CAR_BRAND_REQUEST: "New car brand request",
}